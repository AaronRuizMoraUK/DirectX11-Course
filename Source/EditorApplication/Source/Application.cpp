...
namespace DX
{
    ...
    bool Application::Initialize(const Math::Vector2Int& windowSize, int refreshRate, bool fullScreen, bool vSync)
    {
        ...
        m_window = WindowManager::Get().CreateWindowWithTitle("DirectX11 Course", windowSize, refreshRate, fullScreen, vSync);
        if (!m_window)
        {
            Terminate();
            return false;
        ...
        std::ranges::for_each(m_objects, [scene](auto& object) { scene->AddObject(object.get()); });
